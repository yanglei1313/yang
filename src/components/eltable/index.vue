...
export default {
  ...
  methods: {
        ...
        cols.push(
          h("el-table-column", {
            props: {
              ...
              align: ["selection", "index"].includes(col.type)
                ? "center"
                : this.align,
    ...
    handleTableColumnScopedSlots(h, col) {
      if (["expand", "custom"].includes(col.type)) {
        return {
          default: (attrs) => [
            h(col.component, { props: { parentRow: attrs } }),
          ],
        };
      } else if (col.tag) {
        return {
          default: (attrs) => [
            h("el-tag", { props: { type: col.tag(attrs) } }, attrs.row.tag),
          ],
        };
      } else if (col.actions) {
        return {
          default: (attrs) => {
            ...
            const list =
              typeof col.actions === "function"
                ? col.actions.call(this.$parent, attrs)
                : col.actions;
            ...
            //     },
            //   }),
            //   h("el-dropdown-menu", { slot: "dropdown" }, [arr]),
            // ]);
          },
        };
      }
        ...
        default: (attrs) => [
          h(
            "div",
            {
              class:
                "cell qz_cell" +
                (attrs.column.showOverflowTooltip ? " el-tooltip" : ""),
            },
            (col.labelCallback && col.labelCallback(attrs)) ||
              attrs.row[attrs.column.property]
          ),
        ],
      };
    },
  ...
  render(h) {
    return h(
      "div",
      { class: "qzui-data-table", style: { "margin-top": "20px" } },
      [
        h(
          "el-table",
          {
